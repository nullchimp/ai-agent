...
def test_ui_files_exist():
    ...
    assert (ui_dir / "chat.js").exists(), "chat.js should exist"
...
def test_html_structure():
    ...
    assert '<textarea id="messageInput"' in content, "Should have message input"
    assert '<button id="sendBtn"' in content, "Should have send button"
...
def test_javascript_api_integration():
    ...
    assert 'localhost:8000' in content, "Should have default API endpoint"
...
def test_package_json_scripts():
    ...
    if package_file.exists():
        ...
        assert 'build-ui' in scripts, "Should have build-ui script"
        assert 'serve-ui' in scripts, "Should have serve-ui script"
