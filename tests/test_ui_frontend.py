...
def test_ui_files_exist():
    ...
    assert (dist_dir / "bundle.js").exists(), "bundle.js should exist"
    assert (ui_dir / "main.ts").exists(), "main.ts should exist"
...
def test_html_structure():
    ...
    assert 'bundle.js' in content, "Should include JavaScript file"
...
def test_typescript_compiles():
    ...
    ui_dir = Path(__file__).parent.parent / "src" / "ui"
    ts_file = ui_dir / "main.ts"
    types_file = ui_dir / "types.ts"
    js_file = ui_dir / "dist" / "bundle.js"

    ts_content = ts_file.read_text()
    
    # Check TypeScript structure - interfaces are now in types.ts
    ...
    # Check main.ts imports the types
    assert 'import { Message }' in ts_content, "Should import Message interface"
    assert 'class AgentApp' in ts_content, "Should define AgentApp class"
...
def test_javascript_api_integration():
    ...
    # Check for API-related functionality (these might be minified)
    assert 'fetch(' in content or 'fetch`' in content, "Should have fetch API calls"
...
def test_chat_functionality_structure():
    ...
    # Check for essential methods - these exist in the main app
    ...
    assert 'typingIndicator' in content or 'TypingIndicator' in content, "Should show typing indicators"
    
    # Check for localStorage usage in session management
    session_file = ui_dir / "libs" / "session.ts"
