...
def get_by_id(
    ...
    return get_by_property(label, "id", node_id, fetch_one=True)
...
def get_by_property(
    ...
    with pool.get_connection() as db:
        if issubclass(label, Node) or isinstance(label, Node):
            label = label.label()
...
def delete_by_id(
    ...
    return delete_by_property(label, "id", node_id)
...
def delete_by_property(
    ...
    with pool.get_connection() as db:
        ...
        q = f"MATCH (n:`{label}` {{{property_name}: $value}}) DETACH DELETE n"
        db._cur.execute(q, {"value": property_value})
        db._conn.commit()